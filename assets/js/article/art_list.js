      ...
      .then(({ data: res }) => {
        if (res.code === 0) {
          ...
          res.data.forEach((item) => {
            rows.push(`<tr>
          <td><a href="javascript:;" style="color: blue;" class="show-detail" data-id="${
            item.id
          }">${item.title}</a></td>
      ...
      .then(({ data: res }) => {
        if (res.code === 0) {
          ...
          <div class="artinfo-bar">
            ...
            <span>发布时间：${dayjs(res.data.pub_date).format(
              "YYYY-MM-DD HH:mm:ss"
            )}</span>
          ...
          <hr>
          <img src="http://www.liulongbin.top:3008${
            res.data.cover_img
          }" alt="" class="artinfo-cover">
